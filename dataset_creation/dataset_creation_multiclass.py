...
UNPROCESSED_PATH = r'G:\tesi_4maggio_22\dataset_michele\thesis_single_effect_4_guitars\unprocessed_4guit'
PROCESSED_PATH = r'G:\tesi_4maggio_22\dataset_michele\thesis_single_effect_4_guitars'
...
def generate_equally_spaced_array_shuffled(min, max, length):
    plugin_levels = np.linspace(min, max, length)
    np.random.shuffle(plugin_levels)
    return plugin_levels
...
def prepare_dataset(unprocessed_path, processed_path):
    ...
    reverb_wet_levels = generate_equally_spaced_array_shuffled(min=0.2, max=0.5, length=DATASET_LENGTH)
    chorus_mix_levels = generate_equally_spaced_array_shuffled(min=0.2, max=0.5, length=DATASET_LENGTH)
    distortion_drive_levels = generate_equally_spaced_array_shuffled(min=20, max=40, length=DATASET_LENGTH)
    phaser_mix_levels = generate_equally_spaced_array_shuffled(min=0.2, max=0.5, length=DATASET_LENGTH)
    delay_mix_levels = generate_equally_spaced_array_shuffled(min=0.2, max=0.3, length=DATASET_LENGTH)
    overdrive_gain_levels = generate_equally_spaced_array_shuffled(min=0.2, max=1.0, length=DATASET_LENGTH)
    f_delay_mix_levels = generate_equally_spaced_array_shuffled(min=0.2, max=0.3, length=DATASET_LENGTH)
    flanger_wet_levels = generate_equally_spaced_array_shuffled(min=0.5, max=1.0, length=DATASET_LENGTH)
    vibrato_mix_levels = generate_equally_spaced_array_shuffled(min=0.1, max=0.2, length=DATASET_LENGTH)
    tremolo_mix_levels = generate_equally_spaced_array_shuffled(min=0.3, max=0.6, length=DATASET_LENGTH)
    ...
    for path, folders, files in os.walk(unprocessed_path):
        ...
        for file in files:
            ...
            unprocessed_audio_avoid_clipping = normalize_loudness(unprocessed_audio, SAMPLE_RATE, LOUDNESS_LEVEL)


            # export samples
            export_reverb_sample(file, reverb_wet_levels[count], unprocessed_audio_avoid_clipping, processed_path, sr)
            export_chorus_sample(file, chorus_mix_levels[count], unprocessed_audio_avoid_clipping, processed_path, sr)
            export_distortion_sample(file, distortion_drive_levels[count], unprocessed_audio_avoid_clipping, processed_path, sr)
            export_phaser_sample(file, phaser_mix_levels[count], unprocessed_audio_avoid_clipping, processed_path, sr)
            export_delay_sample(file, delay_mix_levels[count], unprocessed_audio_avoid_clipping, processed_path, sr)
            export_overdrive_sample(file, overdrive_gain_levels[count], unprocessed_audio_avoid_clipping, processed_path, sr, overdrive_vst3)
            export_feedback_delay_sample(file, f_delay_mix_levels[count], unprocessed_audio_avoid_clipping, processed_path, sr)
            export_flanger_sample(file, flanger_wet_levels[count], unprocessed_audio_avoid_clipping, processed_path, sr, flanger_vst3)
            export_vibrato_sample(file, vibrato_mix_levels[count], unprocessed_audio_avoid_clipping, processed_path, sr, vibrato_vst3)
            export_tremolo_sample(file, tremolo_mix_levels[count], unprocessed_audio_avoid_clipping, processed_path, sr, tremolo_vst3)
